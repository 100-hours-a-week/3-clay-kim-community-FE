...
function renderPost(post) {
    ...
    document.getElementById('postDate').textContent = formatDate(
        post.createdAt,
    );
    ...
    authorIcon.innerHTML = `<img src="${profileImageUrl}" alt="프로필" class="post-author-profile-image" onerror="this.onerror=null; this.src='${DEFAULT_PROFILE_IMAGE}';">`;
...
async function handleCommentToggle(page, groupId) {
    try {
        ...
        if (error) {
            ...
            if (group) {
                group.innerHTML =
                    '<div class="comment-empty-text">댓글을 불러오는데 실패했습니다.</div>';
            }
            return;
        }
...
function attachEventListeners() {
    ...
    // 댓글 작성 버튼
    document
        .getElementById('btnCommentSubmit')
        .addEventListener('click', handleCommentSubmit);

    // 댓글 글자 수 카운터
    const commentInput = document.getElementById('commentInput');
    commentInput.addEventListener('input', () => {
        document.getElementById('commentLength').textContent =
...
async function handleDelete() {
    ...
    const { error } = await patch(API_ENDPOINTS.POSTS.DELETE(postId), null, {
        auth: true,
        autoRedirect: true,
    });
